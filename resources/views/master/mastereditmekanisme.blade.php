    ...
    <div class="col-lg-12">
        <div class="card card-default">
            <div class="card-header card-header-border-bottom">
                    <a href="{{ route('mastermekanisme') }}" class="badge badge-primary">Kembali</a>
            </div>
            <div class="card-body">
                <form method="POST" action="{{route('update.mekanisme', $mekanisme->id)}}"  enctype="multipart/form-data">
                    @csrf
                    @method('patch')
                ...
                @enderror</label>
                        ...
                        @if (old('image'))
                         value="{{ $mekanisme->image }}"
                        @else
                         value="{{ $mekanisme->image }}"
                        @endif onchange="loadPreview(this);" name="image"><br>
                    </div>
                    <img id="preview_img" src="{{('/assets/image/' . $mekanisme->image )}}" class="hidden" width="70" height="80"/>
